  ...
  function manualCheckIn(row: ManualRow) {
    startTransition(async () => {
      ...
      setMatches((current) =>
        current.map((item) =>
          item.id === row.id
            ? { ...item, checkedInAt: item.checkedInAt ?? new Date().toISOString() }
            : item
        )
      );
    ...
    <div className="scanner-workspace">
      <section className="camera-panel">
        ...
        <div className="camera-hint">
          Center the student&apos;s QR inside the box. The scanner resets automatically.
        </div>
        {result ? (
          <div className={`scan-result ${result.tone}`}>
            ...
            <div>
              ...
              {result.name ? <strong>{result.name}</strong> : null}
              {result.club ? <small>{result.club}</small> : null}
            </div>
          </div>
        ) : null}
      </section>
          ...
          <div className="manual-body">
            <div className="manual-search">
              <input
                ...
                onKeyDown={(event) => {
                  if (event.key === "Enter") searchManual();
                }}
              ...
              {matches.map((row) => (
                ...
                >
                  <div>
                    ...
                    <span>
                      {row.club ?? "No club"} · {row.phone}
                    </span>
                  </div>
                  <span className={row.checkedInAt ? "manual-status done" : "manual-status"}>
                    {row.checkedInAt ? "Already in" : "Check in"}
                  </span>
                </button>
              ))}
            </div>
